...
#if defined(NDEBUG)
# error "Unknowncoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7b1da5c66a6fefa5ed7441462e1dc018343edf6354e5e578f9f5d5ff7d7ee858");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Unknowncoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Unknowncoin Signed Message:\n";
...
{
    // Unknowncoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 800000); // Unknowncoin: 800k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Unknowncoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Unknowncoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Unknowncoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Unknowncoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6c6d7affb41c8d56723aa275c5e9c0a37292b0c3c987fc56ab66677475d34a61");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "23/Jul/2021 The 2021 Tokyo Olympic Games starts.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0438703284c00bc9cc9cf41dc8cfbc146bcd9fdb3932a21d38fee834e304425e4728f95c7bc1f654d42ba8a596e035cf72b416c662d46717b6b8942ceeae4d208a") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1627645612;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085021604;

        if (fTestNet)
        {
            block.nTime    = 1627645602;
            block.nNonce   = 385367348;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xbc10ed992260a5147aba07d9e95843d94aaff5af931c3af48cda4965c63e01ac"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfu, 0xc8, 0xb3, 0xdf }; // Unknowncoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("UnknowncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UnknowncoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static UnknowncoinMiner(CWallet *pwallet)
{
    printf("UnknowncoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("unknowncoin-miner");
    ...
    try { loop {
        ...
        printf("Running UnknowncoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("UnknowncoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&UnknowncoinMiner, pwallet));
}
